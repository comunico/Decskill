<template>
  <div>
    <h2>
      <span id="metrics-page-heading" data-cy="metricsPageHeading">Métricas de aplicación</span>
      <button class="btn btn-primary float-right" v-on:click="refresh()">
        <font-awesome-icon icon="sync"></font-awesome-icon> <span>Refrescar</span>
      </button>
    </h2>
    ...
    <div class="row" v-if="!updatingMetrics">
      ...
      <div class="col-md-4">
        <h4>Hilos</h4>
        <span><span>Ejecutables</span> {{ threadStats.threadDumpRunnable }}</span>
        ...
        <span><span>Tiempo esperando</span> ({{ threadStats.threadDumpTimedWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
        ...
        <span><span>Esperando</span> ({{ threadStats.threadDumpWaiting }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
        ...
        <span><span>Bloqueados</span> ({{ threadStats.threadDumpBlocked }})</span>
        <b-progress variant="success" :max="threadStats.threadDumpAll" striped>
          <b-progress-bar
      ...
      <div class="table-responsive">
        <table class="table table-striped" aria-describedby="Jvm gc">
          <thead>
            <tr>
              <th scope="col"></th>
              <th scope="col" class="text-right">Contador</th>
    ...
    <h3>Peticiones HTTP (eventos por segundo)</h3>
    <table
    ...
    <h3>Estadísticas de la fuente de datos (tiempo en milisegundos)</h3>
    <div class="table-responsive" v-if="!updatingMetrics && isObjectExistingAndNotEmpty(metrics, 'databases')">
      <table class="table table-striped" aria-describedby="Connection pool">
        <thead>
          <tr>
            <th scope="col">
              <span>Uso</span> (active: {{ metrics.databases.active.value }}, min: {{ metrics.databases.min.value }}, max:
              {{ metrics.databases.max.value }}, idle: {{ metrics.databases.idle.value }})
            </th>
            <th scope="col" class="text-right">Recuento</th>
            <th scope="col" class="text-right">Media</th>
            <th scope="col" class="text-right">Min</th>
            <th scope="col" class="text-right">p50</th>
            <th scope="col" class="text-right">p75</th>
    ...
    <b-modal ref="metricsModal" size="lg">
      <template #modal-title>
        <h4 class="modal-title" id="showMetricsLabel">Volcado de hilos</h4>
      </template>
